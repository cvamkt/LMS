...
export const buySubscription = asyncHandler(async (req, res, next) => {
  ...
  const { id } = req.user;
  console.log("function in the buy subscription",id);
  

  // Finding the user based on the ID
  const user = await User.findById(id);
  console.log("function in the buy subscription",user);
  ...
  console.log("function in the buy subscription",subscription);
  ...
  await user.save();
  console.log("susbcription_id", subscription.id);
  res.status(200).json({
    success: true,
